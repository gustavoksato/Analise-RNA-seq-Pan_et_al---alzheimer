...
adata_completo = sc.read_h5ad("GSE267301_Full_Dataset.h5ad")
...
listafujita_limpa = []
genes_oficial = []
genes_fujita = pd.read_csv('features.tsv.gz')
...
print(listafujita_limpa[:10]) 
len(listafujita_limpa)
adata.var_names
...
for gene in adata.var_names:
    gene_limpo = gene.split(".")[0]
    gene_adata.append(gene_limpo)

adata.var_names = gene_adata

print(gene_adata[:10])
print(adata.var['index1'[:10]])
...
fujita = pd.read_csv('features.tsv.gz')
fujita.shape
